...
''',
    ...
    formula = compile(opts.formula, filename='<formula>', mode='eval')
...
def main():
    ...
    (source_files, options), formula, recruited_judge_names = simple_parse_args([judge._quality_judge_name for judge in judges])
    recruited_judges = [judge for judge in judges if judge._quality_judge_name in recruited_judge_names]

    results = quality.core.run_contest(source_files, options, formula, recruited_judges)
